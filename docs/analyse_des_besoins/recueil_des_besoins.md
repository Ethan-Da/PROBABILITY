...
## V./ Chapitre 5 – Autres exigences 
   ...
   Le projet "Développement d'une application web de calcul" est réalisé par une équipe composée d'ARSENE Maxence, DANGEUL Ethan, CAMARA Blomkoum, LEBLOND Roméo et MAROUAN Hazim-Rayan. Leurs valeurs                 fondamentales pour ce projet sont la simplicité de l'interface, la disponibilité de la plateforme, la rapidité des calculs, la sécurité des accès et des données, ainsi que la souplesse du système pour            permettre son évolution.
   
   Les clients attendent une visibilité régulière sur l'avancement du projet avec des rapports périodiques et des mises à jour. Pour mener à bien ce projet, certains éléments doivent être achetés, comme un          Raspberry Pi 4 et ses accessoires, tandis que la plateforme web elle-même doit être développée, incluant l'infrastructure, la gestion des utilisateurs et les modules de calcul.
   Des exigences techniques et opérationnelles sont à respecter, comme des tests réguliers, l'installation sur un Raspberry Pi 4 avec accès via le réseau local, ainsi qu'une documentation complète pour faciliter    l'utilisation et la maintenance. Le projet dépend également de la fiabilité du serveur, de la connexion réseau et des technologies choisies.

   En termes de règles métier, l'application doit respecter les droits d'accès utilisateurs, valider les données d'entrée pour les calculs et permettre l'enregistrement de l'historique des calculs. La               performance est essentielle, avec un traitement rapide des requêtes, une optimisation des algorithmes de calcul et une gestion efficace des utilisateurs simultanés.
   
   L'application doit garantir une disponibilité optimale, une gestion sécurisée des sessions utilisateurs et une documentation complète. Elle doit également être intuitive et simple d'utilisation, avec une         interface claire et accessible. Enfin, les indicateurs de qualité logicielle, tels que la capacité fonctionnelle, la facilité d'utilisation, la fiabilité, la performance, la maintenabilité et la portabilité,     sont cruciaux pour assurer la durabilité et l'adaptabilité de l'application.
